...
def goto(estado, classe):
    available_classes = list(df_goto.columns)
    available_states = list(df_goto['State'])
    if (estado in available_states and classe in available_classes):
        value = df_goto[classe][estado]
        evaluate = re.sub(r'[0-9]+', '', value)
        print(evaluate)
        if(evaluate != 'e'):
            return df_goto[classe][estado]
        else:
            error_translation(df_goto,estado,classe)
            
            return df_goto[classe][estado]
    else:
        print("Posição [{},{}] inexistente na tabela Go to".format(estado,classe))
...
def action(estado, classe):
    ...
    if (estado in available_states and classe in available_classes):
        ...
        print("Posição [{},{}] inexistente na tabela Action".format(estado,classe))
...
def error_translation(df,estado,classe):
    ...
    print("==> ERRO SINTATICO Linha: {} Coluna: {} => recebeu: {}, disponiveis no estado {}: {}".format(estado,classe,classe,available_list[0],available_list[1:]))
